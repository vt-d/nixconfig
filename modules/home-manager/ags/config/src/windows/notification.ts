// this part is copied from ricecracker-dev's old configuration, will change it later
...
const NotificationImageWidget = (n: Notification) => {
  ...
  return Widget.Box({ className: 'img-notification', children: [icon] });
};
...
const NotificationWindow = () => {
  const list = Widget.Box({
    ...
    hexpand: true,
    spacing: 8,
    children: notifications.popups.map(NotificationW),
  });

  const onNotified = (_: any, id: number) => {
    const n = notifications.getNotification(id)
    if (n) {
      list.children = [NotificationW(n), ...list.children];
    }
  }
